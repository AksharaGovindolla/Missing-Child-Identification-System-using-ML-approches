...
def gen_frames():
    while True:
        ...
        cv2.imwrite('..\\ChildIdentification\\static\\input_images\\captured_image.jpg', frame)
...
def captureimage():
    camera.release()
    return render_template('captured_image.html')

@app.route("/authority_webcam_search2", methods=["GET", "POST"])
def authority_webcam_search2():
    try:
        image_path = "..//ChildIdentification//static//input_images//captured_image.jpg"
        print("img=", image_path)

        if not os.path.exists(image_path):
            return render_template("authority_search.html", message="No image found at the specified path")
        # The image is already saved at the path, so no need to save it again.
        # Instead, directly use it for prediction
        print("Using image for prediction:", image_path)
        # Clear previous test images
        filelist = [f for f in os.listdir("testing")]
        for f in filelist:
            os.remove(os.path.join("testing", f))
        # Copy the image to the testing directory for processing
        imgdata = os.path.basename(image_path)
        test_image_path = os.path.join("testing", imgdata)
        os.rename(image_path, test_image_path)  # Move the file
        ...
        print("Looking for faces in", imgdata)
        predictions = predict(test_image_path, model_path="trained_knn_model.clf")
        ...
        if not namelist or namelist[0] == "unknown":
            ...
            for row in res:
                ...
                test_img= os.path.join("..\\ChildIdentification\\static\\input_images","captured_image.jpg")
                im = Image.open(test_img)
                data = io.BytesIO()
                im.save(data, "JPEG")
                encoded_img_data = base64.b64encode(data.getvalue())
...
@app.route('/captureimage2',methods=["get","post"])
def captureimage2():
    return render_template('captured_image2.html')

@app.route("/user_webcam_search2", methods=["GET", "POST"])
def user_webcam_search2():
    try:
        ...
        if not os.path.exists(image_path):
            return render_template("user_search.html", message="No image found at the specified path")
        # The image is already saved at the path, so no need to save it again.
        # Instead, directly use it for prediction
        print("Using image for prediction:", image_path)
        # Clear previous test images
        ...
        sql2 = "SELECT * FROM uploadphotos WHERE cid=%s"
        cursor.execute(sql2, (namelist[0],))
        res = cursor.fetchall()

        for row in res:
            name1, city1, landmarks, remarks, photo, unm, role = row[:7]

            if role == "Officer":
                sql = "SELECT mno FROM authority WHERE uid=%s"
            else:
                sql = "SELECT mno FROM users WHERE uid=%s"

            cursor.execute(sql, (unm,))
            cno = cursor.fetchone()[0]
            ...
            train_img = os.path.join("dataset", namelist[0], photo)
            im2 = Image.open(train_img)
            data2 = io.BytesIO()
            im2.save(data2, "JPEG")
            encoded_img_data2 = base64.b64encode(data2.getvalue())

            list.extend([name1, city1, landmarks, remarks, f"{unm} ({role})", cno])

        return render_template("results2.html", list=list, img_data=encoded_img_data.decode('utf-8'), img_data2=encoded_img_data2.decode('utf-8'))

    except Exception as e:
        print(e.args[0])
        tb = sys.exc_info()[2]
        print(tb.tb_lineno)
        return render_template("error2.html", error=str(e))
