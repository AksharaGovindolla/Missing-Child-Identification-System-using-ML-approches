...
def gen_frames():
    camera = cv2.VideoCapture(0)  # Open the webcam each time this function is called
    try:
        while True:
...
def authority_webcam_search2():
    try:
        ...
        save_path = '../ChildIdentification/testing/test.jpg'
        ...
        if len(namelist) == 0 or namelist[0] == "unknown":
            ...
            for row in res:
                ...
                test_img= os.path.join("testing",image_file)
                im = Image.open(test_img)
                data = io.BytesIO()
                im.save(data, "JPEG")
                encoded_img_data = base64.b64encode(data.getvalue())
